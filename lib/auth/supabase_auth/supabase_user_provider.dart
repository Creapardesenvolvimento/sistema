...
class CadastroDeEmpresasSupabaseUser extends BaseAuthUser {
  CadastroDeEmpresasSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> cadastroDeEmpresasSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CadastroDeEmpresasSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
